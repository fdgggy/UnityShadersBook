Shader "Unity Shaders Book/Chapter 7/Normal Map In Tangent Space" { //切线空间下计算
	Properties {
...
//				o.viewDir = mul(rotation, normalize(ObjSpaceViewDir(v.vertex))).xyz;
			...
			fixed4 frag(v2f i) : SV_Target {				
				...
				fixed4 packedNormal = tex2D(_BumpMap, i.uv.zw);//对法线纹理采样获得纹素
...
//				tangentNormal.z = sqrt(1.0 - saturate(dot(tangentNormal.xy, tangentNormal.xy)));
				...
				tangentNormal = UnpackNormal(packedNormal);//把法线纹理中存储的像素值反映射回法线
				tangentNormal.xy *= _BumpScale;//乘以凹凸程度，0就是不凹凸
				tangentNormal.z = sqrt(1.0 - saturate(dot(tangentNormal.xy, tangentNormal.xy)));//计算发现z的分量???
